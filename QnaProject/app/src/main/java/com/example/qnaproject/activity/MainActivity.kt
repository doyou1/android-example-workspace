...
class MainActivity:AppCompatActivity() {
    ...
    private fun setSpinableText() {
        val sp = SpannableStringBuilder("로그인하시면 ")
        sp.append("개인정보취급방침")
        ...
        }, sp.length - "개인정보취급방침".length, sp.length, 0)
        sp.append(" 및 ")
        sp.append("이용약관")
        sp.setSpan(object: ClickableSpan() {
            override fun onClick(p0: View) {
        ...
        }, sp.length - "이용약관".length, sp.length, 0)

        sp.append(" 동의하게 됩니다.")
        binding.tvPolicy.movementMethod = LinkMovementMethod.getInstance()
        binding.tvPolicy.setText(sp, TextView.BufferType.SPANNABLE)
    }


    private fun openPrivacy() {
        val intent = Intent(Intent.ACTION_VIEW, Uri.parse("hellobiz://webview?type=privacy"))
        startActivity(intent)
    }
    private fun openTerms() {
        val intent = Intent(Intent.ACTION_VIEW, Uri.parse("hellobiz://webview?type=terms"))
        startActivity(intent)
    }
